...
{
    $query = "SELECT user_id ,user_fname ,user_lname ,email ,user_address,membership_status FROM user WHERE user_id=$id";
    $data = query($query);
    return $data;
}
...
function add_cart($item_id)
{
    ...
    $quantity = isset($_GET['quantity']) ? $_GET['quantity'] : 1; // Set default quantity to 1 if not provided
    if (empty($user_id)) {
